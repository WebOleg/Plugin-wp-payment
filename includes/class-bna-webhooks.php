...
class BNA_Webhooks {
    ...
    public static function handle_webhook($request) {
        ...
        bna_log('Webhook received - FULL DEBUG', array(
    ...
    private static function handle_payment_method_webhook($payload) {
        ...
        } elseif (!empty($payload['customer']['id'])) {
            $customer_id = $payload['customer']['id'];
        } elseif (!empty($payload['customerId'])) {
    ...
    private static function handle_transaction_webhook($payload) {
        ...
        if (empty($transaction['id'])) {
            bna_error('Invalid transaction structure in webhook', array(
                'payload_keys' => array_keys($payload),
                'transaction_keys' => is_array($transaction) ? array_keys($transaction) : 'not_array'
            ));
    ...
    private static function determine_payment_type($payment_data) {
        ...
        if (isset($payment_data['method'])) {
            ...
            if ($method === 'card') {
                return isset($payment_data['cardType']) ? strtoupper($payment_data['cardType']) : 'CREDIT';
            }
